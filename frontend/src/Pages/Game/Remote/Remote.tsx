...
const tableUrl = new URL("../../../assets/glb/tableLwa3ra.glb", import.meta.url)
	.href;
...
interface ResultContext {
	...
	setWinner: React.Dispatch<
		React.SetStateAction<userDataType | FriendDataType | null>
	>;
}
...
function Ball({ ball }: { ball: [number, number, number] }) {
	...
	useEffect(() => {
		if (api && api.position) api.position.set(ball[0], ball[1], ball[2]);
	}, [ball, api]);
	return (
		<>
...
function Paddle1({ position, box }: Paddlerops) {
	// my Paddle
...
function SideWall({ position }: SideWallProps) {
	...
	return (
		<mesh ref={ref} position={position} name="side_wall" visible={false}>
			<boxGeometry args={[0.5, 0.8, 8.6564]} />
			<meshStandardMaterial />
		</mesh>
	);
}
		...
		useEffect(() => {
			if (!socketRef || !socketRef.current)
				socketRef.current = new WebSocket(
					getendpoint("ws", `/ws/game/remote/${gameId}`)
				);
			const socket = socketRef.current;
			socket.onmessage = (e) => {
				...
				if (data.event == "WINNER") {
					if (data.winner == user.username) setWinner(user);
					//need tn9ah
		...
		if (!loading)
			return (
				<>
					<Table />
					<Ball ball={ball} />
					<Paddle1 position={[0, 0.09, +(8.6564 - 0.6) / 2]} box={paddle1} />
					<Paddle2 position={[0, 0.09, -(8.6564 - 0.6) / 2]} box={paddle2} />
					<SideWall position={[(6.1469 + 0.1) / 2, 0, 0]} />
					<SideWall position={[-(6.1469 + 0.1) / 2, 0, 0]} />
					<GoalWall position={[0, 0, (8.6564 + 0.5) / 2]} />
					<GoalWall position={[0, 0, -(8.6564 + 0.4) / 2]} />
				...
				<div className="PlayScreen">
					...
					{winner ? (
						<div className="winnerPopUp">
							...
							<img
								src={getendpoint("http", winner.avatar)}
...
const Provider = ({
	...
	const [winner, setWinner] = useState<userDataType | FriendDataType | null>(
		null
	);
...
const Remote = () => {
	...
	const [gameId] = useState<number>(
		id && !isNaN(parseInt(id, 10)) ? parseInt(id, 10) : -1
	);
	if (gameId == -1) return <Navigate to={"/"} />;
